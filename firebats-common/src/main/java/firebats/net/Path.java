...
public class Path implements Cloneable {
	...
	public int matchingFirstSegments(Path anotherPath) {
		Preconditions.checkNotNull(anotherPath);
	...
	public Path setDevice(String value) {
		if (value != null) {
			Preconditions.checkArgument(value.indexOf(Path.DEVICE_SEPARATOR) == (value.length() - 1), "Last character should be the device separator"); //$NON-NLS-1$
		}
	...
	public Path uptoSegment(int count) {
		...
		if (count >= segments.length)
			return this;
		Preconditions.checkArgument(count > 0, "Invalid parameter to Path.uptoSegment"); //$NON-NLS-1$
